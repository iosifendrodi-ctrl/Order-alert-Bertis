...
# v8 — canonical shortage calculation and severity
def calculate_shortage_pct(ordered, picked):
    ordered = float(ordered or 0)
    picked = float(picked or 0)
    if ordered <= 0:
        return 0.0
    return max(0.0, (ordered - picked) / ordered * 100.0)

def calculate_severity(shortage_pct, warning_threshold, critical_threshold):
    p = float(shortage_pct or 0)
    w = float(warning_threshold or 0)
    c = float(critical_threshold or 0)
    if c < w:
        w, c = c, w
    if p >= c:
        return "critical"
